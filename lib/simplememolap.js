...
var simplememolap = (function () {
    ...
    BaseCursor.prototype.group = function (fields, fn, inistate) { 
        return new GroupCursor(this, fields, fn, inistate); 
    }

    function GroupCursor(cursor, fields, fn, inistate) {
        ...
        var newdimensions = new Array(nfields);

        for (var k = 0; k < nfields; k++) {
            dimoffsets[k] = getDimensionOffset(fields[k], dimensions);
            newdimensions[k] = dimensions[dimoffsets[k]];
        }
        ...
        this.next = function () {
            ...
            var data = grouper[key];
                
            var tuple = keyToTuple(key);
            tuple.data = data;
            
            return tuple;
        };
        
        this.dimensions = function () { return newdimensions; }
        ...
        function group() {
            ...
            while (tuple = cursor.next()) {
                var values = getValues(tuple);
                var key = valuesToKey(values);
                
                if (!grouper[key])
                    grouper[key] = inistate;
                    
                grouper[key] = fn(tuple.data, grouper[key]);
            }
            
            keys = Object.keys(grouper);
            nkeys = keys.length;
            position = 0;
        };
    }
    
    GroupCursor.prototype.__proto__ = BaseCursor.prototype;
    ...
    function FilterCursor(cursor, filter) {
        ...
        this.dimensions = function () { return cursor.dimensions() };
    }
    
    FilterCursor.prototype.__proto__ = BaseCursor.prototype;
